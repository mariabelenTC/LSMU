package marbel.mov.urjc.colorea_marbel;
...
public class MainActivity extends AppCompatActivity {
    ...
    private int tiempo=120000; // tiempo inicial en milisegundos -> 2 minuto = 2+60*1000
    ...
    private int score(int color_original, int color_asignado){
        ...
        Log.v(TAG2, ("resta: " + Float.toString(resta)));
        float score = (float)((-510/255)* (int)resta +510);
        //float score = (float) ((-100/255)*resta+100);
        Log.v(TAG2, ("calcular score: " + Integer.toString((int)score)));
    ...
    private void getScore(){
        ...
        for(int i = 0; i < fila; i++) {
            for (int j = 0; j < columna; j++) {
                ...
                Log.v(TAG2, ("red: " + Integer.toString(r_p) +
                        " green: " + Integer.toString(g_p) +
                        " blue: " + Integer.toString(b_p)));


                colorR_score = score(r, r_p);
                colorG_score = score(g, g_p);
                colorB_score = score(b, b_p);
                Log.v(TAG2, ("score_R: " + Integer.toString(colorR_score) +
                ...
                Log.v(TAG2, ("score del boton: " + Integer.toString(score_boton)));
        ...
        if (total_score>= ((score_max *8)/9)){
            Log.v(TAG1,("has ganado y tu puntación es: " + Integer.toString(total_score)));
            ...
            Log.v(TAG1,("has perdido y tu puntación es: " + Integer.toString(total_score)));
    ...
    private void setGridLayoutButtons(GridLayout gridLayout, Bitmap bm) {
        ...
        for(int i = 0; i < bm.getHeight(); i++){
            for(int j =0; j < bm.getWidth(); j++){
                ...
                //Asigna al botón el color blanco
                int color = Color.rgb(255 ,255,255 );
                b.setBackground(new ColorDrawable(color_original));
